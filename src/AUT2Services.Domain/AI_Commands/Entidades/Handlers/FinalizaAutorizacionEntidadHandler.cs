...
namespace AUT2Services.Domain.Commands.Entidades.Handlers
{
    public partial class EntidadCommandHandler :
        IRequestHandler<FinalizaAutorizacionEntidadCommand, CommandResponse>
    {
        public async Task<CommandResponse> Handle(FinalizaAutorizacionEntidadCommand command, CancellationToken cancellationToken)
        {
        ...
        newEntidad.CambiarFechaTerminoAutorizacion(_clock.UtcNow);

            AddUpdateDomainEvent(command, newEntidad, new EntidadEventAutorizacionFinalizada(
                    newEntidad.Id, 
                    newEntidad.FechaTerminoAutorizacion 
