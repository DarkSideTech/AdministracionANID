// -------------------------------------------------
// Dark Side Tech
// Solution Name : AUT2Services
// Domain : Administracion version 1.17
// Date Generated File : 2026-04-05 15:14:10.079
// -------------------------------------------------
using AUT2Services.Domain.Core.Enumerations;

namespace AUT2Services.Domain.Enumerations;

public static class EnumTipoDeEntidad
{
    public const string PERSONA = nameof(PERSONA);
    public const string UNIDAD_ORGANIZACIONAL = nameof(UNIDAD_ORGANIZACIONAL);

    public static IList<string> ObtenerListaValores()
    {
        return EnumUtils.GetAllPublicConstantValues<string>(typeof(EnumTipoDeEntidad));
    }
}

